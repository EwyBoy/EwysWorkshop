...
import com.ewyboy.ewysworkshop.gui.container.slot.SlotUnitCraftingOutput;
...
public class TileEntityTable extends TileEntity implements IInventory, ISidedInventory, IFluidHandler, IEnergyHandler {
	...
	public TileEntityTable() {
		...
		pages.add(new PageMain(this, StringMap.PageMain));
		pages.add(new PageTransfer(this, StringMap.PageTransfer));
		pages.add(new PageUpgrades(this, StringMap.PageUpgrade));
	...
	private void transfer(Setting setting, Side side, Transfer transfer, int transferSize) {
		if (transfer.isEnabled() && transfer.isAuto()) {
			ForgeDirection direction = ForgeDirection.values()[BlockWorkshopTable
					.getSideFromSideAndMetaReversed(side.getDirection().ordinal(), getBlockMetadata())];
			TileEntity te = worldObj.getTileEntity(xCoord + direction.offsetX, yCoord + direction.offsetY,
			...
			if (te instanceof IInventory) {
				...
				if (transfer.isInput()) {
					transfer(inventory, this, slots2, slots1, directionReversed.ordinal(), direction.ordinal(),
							transferSize);
				} else {
					transfer(this, inventory, slots1, slots2, direction.ordinal(), directionReversed.ordinal(),
							transferSize);
				}
			}
		}
	...
	private void transfer(IInventory from, IInventory to, int[] fromSlots, int[] toSlots, int fromSide, int toSide,
			int maxTransfer) {
		int oldTransfer = maxTransfer;

		try {
			ISidedInventory fromSided = fromSide != -1 && from instanceof ISidedInventory ? (ISidedInventory) from
					: null;
	...
	private int getTransferSide(int side) {
		return BlockWorkshopTable.getSideFromSideAndMeta(side, getBlockMetadata());
	}
	...
	public void spitOutItem(ItemStack item) {
		...
		EntityItem entityItem = new EntityItem(worldObj, xCoord + offsetX, yCoord + offsetY, zCoord + offsetZ,
				item.copy());
